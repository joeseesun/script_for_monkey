// ==UserScript==
// @name         Translate X Post with Gemini (Advanced Inline Translation)
// @namespace    http://tampermonkey.net/
// @version      2.5
// @description  Dynamically translate X posts with advanced processing and insert results below original text
// @author       You
// @match        https://x.com/*
...
const PROMPT = `
处理说明：
1. 如果文本非中文，翻译成简体中文，并只返回翻译结果，不包含任何解释或额外信息。
2. 如果是中文，站在客观角度点评这个帖子，限制50字。
3. 如果文本是英文，除翻译外，提炼3个值得学习的单词或词汇，给出每个单词的翻译和解释，限制50字。
...
// 工具函数：设置翻译结果到元素
function setTranslation({ element, processedText }) {
    ...
    // 解析并格式化输出，去除 Markdown 格式，使用换行和分隔线
    const lines = processedText.split('\n').map(line => line.trim()).filter(line => line);
    let formattedContent = '';
    for (let i = 0; i < lines.length; i++) {
        if (lines[i] === '翻译') {
            formattedContent += lines[i + 1] || '';
        } else if (lines[i] === '点评（如果有）') {
            if (lines[i + 1]) {
                formattedContent += '\n------------------------\n' + lines[i + 1];
            }
        } else if (lines[i] === '词汇') {
    ...
    translationContainer.textContent = formattedContent.trim(); // 只显示格式化后的纯文本
        ...
        onload: function(response) {
            ...
            if (response.status === 200) {
                try {
                    ...
                    const processedText = responseJson.candidates?.[0]?.content?.parts?.[0]?.text || 'Translation failed';
                    console.log('Parsed processed text:', processedText);
                    setTranslation({ element: originalElement, processedText });
                } catch (e) {
